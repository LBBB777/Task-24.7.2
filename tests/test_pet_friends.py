...
########################################################################################
print('Т1: ЗАПУСКАЮ тест 1 tests_get_api_key_for_valid_user// ПРОВЕРИТЬ АУТЕНТИФИКАЦИЮ')
...
def tests_get_api_key_for_valid_user(email=valid_email, password=valid_password):
    ...
    if status == 200:
        print("Тест 1- Status 200")
    else:
        print(f'Тест 1-{status}')
    if 'key' in result:
        print(f'Тест 1-key:{result['key']}')
    else:
        print(f'Тест 1-key:{'KEY NOT FOUND'}')
tests_get_api_key_for_valid_user()
#############################################################################################
print('Т2: ЗАПУСКАЮ тест 2 tests_get_all_pets_with_valid_key//GET-получить список питомцев')
...
def tests_get_all_pets_with_valid_key(filter=''):
    ...
    if status == 200:
        print("Тест 2- Status 200")
    else:
        print (f'Тест 2-{status}')
...
#########################################################################################
print('Т3: ЗАПУСКАЮ тест 3 tests_post_add_new_pet//POST- РАЗМЕТИТЬ НОВОГО ПИТОМЦА')
...
def tests_post_add_new_pet(name, animal_type, age):
    ...
    print(f'Переменная auth_key: {auth_key}')
    print(f'Ключ в переменной auth_key: {auth_key['key']}')
    # Добавляем нового питомца
    result, status = pf.add_new_pet(auth_key, name, animal_type, age )
    if status == 200:
        print("Тест 3 - Status 200")
    else:
        print(f'Тест 3-{status}')
...
############################################################################
print('Т4: ЗАПУСКАЮ тест 4 tests_delete_add_new_pet//DELETE')
...
def test_successful_delete_self_pet(pytest=None):
    ...
    print('Список питомцев:', my_pets)  # Если список пуст, то получим сообщение:"Список питомцев: {'pets': []}"
    ...
    my_pets, _ = pf.get_list_of_pets(auth_key['key'], "my_pets")
    print(f'(ИТОГОВЫЙ СПИСОК МОИХ ПИТОМЦЕВ/ содержание my_pets: {my_pets}')
    # print(f'(Содержание result: {result}')
...
############################################################################
print('Т5: ЗАПУСКАЮ тест 5 test_successful_update_self_pet_info//PUT - ОБНОВЛЕНИЕ ИНФОЛРМАЦИИ О ПИТОМЦЕ')
def test_successful_update_self_pet_info(name='Мурзик', animal_type='Котэ', age='1'):
...
################## "ЭТО БАЗА ТЕСТОВ. ДАЛЬШЕ НЕ ПИСАТЬ!!!!!!!!!!!!!!!!!!!!!!!!####################
